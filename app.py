...
def scrape_zonaprop(tipo, operacion, localidad, ambientes, es_usado, job_id, max_pages=8):
    ...
    while page <= max_pages:
        update_job(job_id, status=f'Scrapeando ZonaProp página {page}...')
...
def scrape_argenprop(tipo, operacion, localidad, ambientes, es_usado, job_id, max_pages=8):
    tipo_slug = TIPO_SLUG_AP.get(tipo, 'departamentos')
    op_slug = 'venta' if operacion == 'venta' else 'alquiler'
    loc_slug = BARRIO_SLUG.get(localidad, localidad.lower().replace(' ', '-'))
    amb_slug = 'monoambiente' if ambientes == '1' else f'{ambientes}-ambientes'

    listings = []
    page = 1

    while page <= max_pages:
        update_job(job_id, status=f'Scrapeando Argenprop página {page}...')
...
def filter_listings(listings, tipo, m2_objetivo, antiguedad_sel, es_usado, ref_coords, localidad, job_id):
    stats = {
        'total': len(listings),
        'excluidos_nuevos': 0,
        'excluidos_m2': 0,
    ...
    # Filtro distancia con geocoding
    if ref_coords:
        update_job(job_id, status='Calculando distancias...')
...
def calcular_precio_equivalente(m2_cubiertos, amenities):
    ...
    if amenities.get('jardin_m2') and m2_cubiertos:
        jardin = float(amenities['jardin_m2'])
        pct_jardin = jardin / float(m2_cubiertos)
        if pct_jardin <= 0.12:
            m2_eq += jardin * 1.0
...
def calcular_tasacion(listings, m2_cubiertos, amenities):
    ...
    precios_m2 = sorted([l['usd_m2'] for l in listings if l.get('usd_m2')])
    precios = sorted([l['precio'] for l in listings if l.get('precio')])
...
def run_tasacion(job_id, data):
    try:
        ...
        ref_coords = None
        if referencia:
            update_job(job_id, status='Geocodificando ubicación de referencia...')
            query = f"{referencia}, {localidad}, Buenos Aires"
            ref_coords = geocode(query)

        # Scraping ZonaProp
        zp_listings = scrape_zonaprop(tipo, operacion, localidad, ambientes, es_usado, job_id)

        # Scraping Argenprop
        ap_listings = scrape_argenprop(tipo, operacion, localidad, ambientes, es_usado, job_id)

        all_listings = zp_listings + ap_listings
        update_job(job_id, status=f'Aplicando filtros a {len(all_listings)} publicaciones...')

        # Filtros
        filtered, stats = filter_listings(
            all_listings, tipo, m2_cubiertos, antiguedad_sel,
            es_usado, ref_coords, localidad, job_id
        )

        # Tasación
        update_job(job_id, status='Calculando tasación...')
        ...
        update_job(job_id, state='error', status=f'Error: {str(e)}')
...
def tasar():
    ...
    thread = threading.Thread(target=run_tasacion, args=(job_id, data))
    thread.daemon = True
    thread.start()
...
def get_job(job_id):
    job = jobs.get(job_id)
    if not job:
        return jsonify({'state': 'error', 'status': 'Job no encontrado'}), 404
    return jsonify(job)
